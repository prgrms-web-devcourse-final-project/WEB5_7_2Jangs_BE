...
- PR 제목과 본문, Issue, README, 설계 문서, 운영 문서, 성능 보고서와 리뷰 요약은 최종 제출 전에 `humanize-korean`으로 윤문한다.
- 사실, 수치, 코드 식별자, 명령어, API 계약과 인용문은 변경하지 않는다.
- 기술적 정확성을 먼저 검증한 뒤 문체만 다듬는다.
- 커밋 메시지는 `<type>: <한국어 작업 요약>` 형식을 사용하고, 짧은 메시지에는 전체 `humanize-korean` 워크플로우를 실행하지 않는다.
- `humanize-korean`을 사용할 수 없는 환경에서는 같은 원칙으로 직접 검토하며 작업을 중단하지 않는다.
- `_workspace/` 산출물은 Git에 커밋하지 않는다.
- `_workspace/` 산출물 폴더는 `YYYY-MM-DD-<문서-주제>-<순번>` 형식으로 이름을 정해, 폴더명만 보고도 문서 성격을 알 수 있게 한다. 예: `_workspace/2026-08-14-staging-dev-merge-review-001/`.
...
- `perf/`는 `도메인 → 시나리오 → 결과` 순서로 구성한다. 예: `perf/read/commit-cache/results/`, `perf/delete/single/document/results/`.
- 실행 파일과 해당 테스트는 시나리오 디렉터리에 두고, 공용 데이터셋 생성기는 `perf/seed/`에 둔다.
- `results/`에는 해당 시나리오의 raw 결과만 둔다. 사람이 읽는 최종 판정과 비교 보고서는 `docs/performance/`에 둔다.
- raw 결과는 기본적으로 Git에서 제외한다. 결론을 재계산하는 데 필요한 작은 비민감 산출물만 `results/evidence/`에 커밋할 수 있다.
- 새 결과 파일명은 `<scenario>-<environment>-<profile>-r<run>` 형식을 우선한다. 대상이 여러 개인 시나리오는 결과 디렉터리로 먼저 분리한다.
- 새 성능 시나리오에는 실행 조건, 결과 위치, 데이터 정리 방법을 README에 기록한다.
...
- 커밋 메시지는 기존 관례인 `<type>: <한국어 작업 요약>` 형식을 사용한다. 예: `refactor: 성능 측정 디렉터리 구조 정리`.
- `type`은 작업 성격에 맞춰 `feat`, `fix`, `refactor`, `test`, `docs`, `perf`, `chore` 중 하나를 소문자로 사용한다.
- 하나의 커밋에는 하나의 논리적 변경만 담는다. 구현, 검증 도구, 결과 문서가 하나의 변경을 완성하는 경우에는 함께 담을 수 있다.
